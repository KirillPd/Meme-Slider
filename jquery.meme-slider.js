;(function ($) {
  ...
  class MemeSlider {
    constructor($slider, options) {
      this.options = Object.assign(true, {}, $.fn.memeSlider.defaults, options);
      this.$slider = $slider;
...
$(".meme-slider-1").memeSlider({
  ...
  items: 2,
  onSlideChangeEnd: function () {
    console.log("onSlideChangeEnd Fired");
  },
  onInit: function () {
    let $wrapper = this.$wrapper;
    ...
    $wrapper.css({
      position: "relative",
      height: this.slides.$collection.innerHeight()
    });
    ...
    this.animation = {
      fadeIn: ($slides, speed) => {
        ...
        $slides.filter(`.${this.options.slideActiveClass}`).css({
          opacity: 1
        });
      }
    };

    this.animation.fadeIn(this.slides.$collection, 1200);
  },
  animationFunction: function (newIndex, onSlideChangeEnd) {
    let animationSpeed = 1200;

    this.animation.fadeIn(this.slides.$collection, animationSpeed);
  ...
  animationFunctionOnResize: function () {}
});
